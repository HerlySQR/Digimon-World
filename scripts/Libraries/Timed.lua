...
Timed = {
    ...
    echo = function(timeout, duration, userFunc, onExpire, tolerance)
        ...
        timeout = timeout or _DEFAULT_ECHO_TIMEOUT
        tolerance = tolerance or _EXIT_WHEN_FACTOR
        local tm = CreateTimer()
        TimerStart(tm, timeout, true, function()
            local remove = false
            local success, result = xpcall(userFunc, print)
            if success then
                remove = result == true
            end
            if not remove and duration then
                duration = duration - timeout
                local exitwhen = timeout * tolerance
                if duration < exitwhen then
                    remove = true
                    if onExpire then
                        onExpire()
        ...
        return function(doNotDestroy)
            local remaining = TimerGetRemaining(tm)
            if not doNotDestroy then
                PauseTimer(tm)
                DestroyTimer(tm)
            end
