...
setmetatable(_G, {
    __index = function(_, n)
        if tostring(n):sub(1, 3) ~= 'bj_' and tostring(n):sub(1, 4) ~= 'udg_'then
            print(GetStackTrace())
            print("Trying to read undeclared global : " .. tostring(n)) --Shows a message ingame, when a non-declared global is used.
        end
    end})
